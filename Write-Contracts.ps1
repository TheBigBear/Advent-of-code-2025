...
# --- Contract contents ---
# NOTE: content is kept EXACTLY as in your working baseline (no triple backticks; 4-space code blocks)
$pythonContract = @'
...
# --- EOL normalization to ensure exact checksum match and preserve blank lines ---
function Normalize-Text {
  ...
  if ($t -notmatch "`n$") { $t += "`n" }
  # Convert to platform newline (LF on macOS/Linux, CRLF on Windows)
...
$pyNormalized = Normalize-Text $pythonContract
$psNormalized = Normalize-Text $powershellContract

# --- Write files (UTF-8, no BOM) using .NET, which preserves content exactly as provided ---
[System.IO.File]::WriteAllText($pyPath, $pyNormalized, [System.Text.Encoding]::UTF8)
[System.IO.File]::WriteAllText($psPath, $psNormalized, [System.Text.Encoding]::UTF8)
...
# --- Compute expected (from normalized strings) and actual (from files) hashes ---
$expectedPySHA256 = Get-StringHashHex -Text $pyNormalized -Algorithm 'SHA256'
$expectedPsSHA256 = Get-StringHashHex -Text $psNormalized -Algorithm 'SHA256'
